//! Wyrd Mind local development and automation CLI.

#![forbid(unsafe_code)]

use anyhow::{Context, Result, bail};
use std::{
    env, fs,
    io::{self, Read},
    net::SocketAddr,
    path::PathBuf,
    process::Command,
};
use wyrd_mind_core::{Database, NewFixImport, NewRepo, NewReviewSession};

#[tokio::main]
async fn main() -> Result<()> {
    let mut args = env::args().skip(1);
    let command = args.next().unwrap_or_else(|| "help".to_string());
    let db = Database::new(database_path()?);

    match command.as_str() {
        "help" | "-h" | "--help" => {
            print_help();
            Ok(())
        }
        "db" => match args.next().as_deref() {
            Some("migrate") => {
                db.migrate()?;
                println!("migrated {}", db.path().display());
                Ok(())
            }
            Some("reset") => {
                db.reset()?;
                println!("reset {}", db.path().display());
                Ok(())
            }
            _ => bail!("usage: wyrd-mind-cli db <migrate|reset>"),
        },
        "repo" => match args.next().as_deref() {
            Some("add") => {
                let path = args.next().context("missing repo path")?;
                let name = args.next().unwrap_or_else(|| repo_name(&path));
                db.migrate()?;
                let repo = db.upsert_repo(NewRepo { name, path })?;
                println!("{}", serde_json::to_string_pretty(&repo)?);
                Ok(())
            }
            Some("list") => {
                db.migrate()?;
                println!("{}", serde_json::to_string_pretty(&db.repos()?)?);
                Ok(())
            }
            _ => bail!("usage: wyrd-mind-cli repo <add|list>"),
        },
        "review" => match args.next().as_deref() {
            Some("create") => {
                let repo_id = args.next().context("missing repo id")?;
                let base_ref = args.next().context("missing base ref")?;
                let head_ref = args.next().context("missing head ref")?;
                let title = args
                    .next()
                    .unwrap_or_else(|| format!("{base_ref}..{head_ref}"));
                db.migrate()?;
                let session = db.create_review_session(NewReviewSession {
                    repo_id,
                    title,
                    base_ref,
                    head_ref,
                })?;
                println!("{}", serde_json::to_string_pretty(&session)?);
                Ok(())
            }
            Some("context") => {
                let session_id = args.next().context("missing session id")?;
                db.migrate()?;
                println!(
                    "{}",
                    serde_json::to_string_pretty(&db.agent_context(&session_id)?)?
                );
                Ok(())
            }
            Some("record-fix") => {
                let session_id = args.next().context("missing session id")?;
                let commit_sha = args.next().context("missing commit sha")?;
                let response_arg = args.next();
                let tests_arg = args.next();
                db.migrate()?;
                let id = db.import_fix(NewFixImport {
                    session_id,
                    commit_sha,
                    agent_name: Some(
                        env::var("WYRD_MIND_AGENT").unwrap_or_else(|_| "codex".to_string()),
                    ),
                    response_text: read_optional_text(response_arg)?,
                    tests_json: read_optional_text(tests_arg)?,
                    accepted: true,
                })?;
                println!("{id}");
                Ok(())
            }
            _ => bail!("usage: wyrd-mind-cli review <create|context|record-fix>"),
        },
        "hook" => match args.next().as_deref() {
            Some("agent-stop") => {
                db.migrate()?;
                record_agent_stop(&db)
            }
            _ => bail!("usage: wyrd-mind-cli hook agent-stop"),
        },
        "export" => match args.next().as_deref() {
            Some("trajectory") => {
                let repo_id = args.next();
                db.migrate()?;
                for record in db.trajectory_records(repo_id.as_deref())? {
                    println!("{}", serde_json::to_string(&record)?);
                }
                Ok(())
            }
            _ => bail!("usage: wyrd-mind-cli export trajectory [repo_id]"),
        },
        "serve" => {
            db.migrate()?;
            let host = env::var("WYRD_MIND_API_HOST").unwrap_or_else(|_| "127.0.0.1".to_string());
            let port = env::var("WYRD_MIND_API_PORT")
                .unwrap_or_else(|_| "8765".to_string())
                .parse::<u16>()?;
            let token =
                env::var("WYRD_MIND_TOKEN").unwrap_or_else(|_| "dev-local-token".to_string());
            let addr: SocketAddr = format!("{host}:{port}").parse()?;
            println!("wyrd-mind API listening on http://{addr}");
            println!("authorization: Bearer {token}");
            wyrd_mind_api::serve(db, token, addr).await
        }
        other => bail!("unknown command: {other}"),
    }
}

fn database_path() -> Result<PathBuf> {
    if let Ok(url) = env::var("DATABASE_URL")
        && let Some(path) = url.strip_prefix("sqlite://")
    {
        return Ok(PathBuf::from(path));
    }
    Ok(PathBuf::from(".data/wyrd-mind.db"))
}

fn repo_name(path: &str) -> String {
    PathBuf::from(path)
        .file_name()
        .and_then(|value| value.to_str())
        .unwrap_or("repo")
        .to_string()
}

fn read_optional_text(input: Option<String>) -> Result<Option<String>> {
    match input.as_deref() {
        None => Ok(None),
        Some("-") => {
            let mut value = String::new();
            io::stdin().read_to_string(&mut value)?;
            Ok(Some(value))
        }
        Some(path) => fs::read_to_string(path)
            .with_context(|| format!("failed to read {path}"))
            .map(Some),
    }
}

fn record_agent_stop(db: &Database) -> Result<()> {
    let Ok(session_id) = env::var("WYRD_MIND_SESSION_ID") else {
        drain_stdin()?;
        println!("wyrd-mind hook skipped: WYRD_MIND_SESSION_ID is not set");
        return Ok(());
    };
    if session_id.trim().is_empty() {
        drain_stdin()?;
        println!("wyrd-mind hook skipped: WYRD_MIND_SESSION_ID is empty");
        return Ok(());
    }

    let commit_sha = env::var("WYRD_MIND_COMMIT_SHA").unwrap_or_else(|_| current_head());
    if commit_sha.is_empty() {
        drain_stdin()?;
        println!("wyrd-mind hook skipped: git HEAD is unavailable");
        return Ok(());
    }
    if env::var("WYRD_MIND_START_SHA").is_ok_and(|start| start == commit_sha) {
        drain_stdin()?;
        println!("wyrd-mind hook skipped: HEAD has not changed");
        return Ok(());
    }
    if db.fix_import_exists(&session_id, &commit_sha)? {
        drain_stdin()?;
        println!("wyrd-mind hook skipped: {commit_sha} already recorded for {session_id}");
        return Ok(());
    }

    let response_text = if let Ok(path) = env::var("WYRD_MIND_RESPONSE_FILE") {
        read_optional_text(Some(path))?
    } else {
        let value = drain_stdin()?;
        (!value.trim().is_empty()).then_some(value)
    };
    let tests_json = env::var("WYRD_MIND_TESTS_FILE")
        .ok()
        .map(fs::read_to_string)
        .transpose()
        .context("failed to read WYRD_MIND_TESTS_FILE")?;
    let accepted = env::var("WYRD_MIND_ACCEPTED").map_or(true, |value| value != "false");
    let agent_name = Some(env::var("WYRD_MIND_AGENT").unwrap_or_else(|_| "agent".to_string()));

    let id = db.import_fix(NewFixImport {
        session_id,
        commit_sha,
        agent_name,
        response_text,
        tests_json,
        accepted,
    })?;
    println!("wyrd-mind hook recorded fix import {id}");
    Ok(())
}

fn drain_stdin() -> Result<String> {
    let mut value = String::new();
    io::stdin().read_to_string(&mut value)?;
    Ok(value)
}

fn current_head() -> String {
    let Ok(output) = Command::new("git").args(["rev-parse", "HEAD"]).output() else {
        return String::new();
    };
    if !output.status.success() {
        return String::new();
    }
    String::from_utf8_lossy(&output.stdout).trim().to_string()
}

fn print_help() {
    println!(
        "\
wyrd-mind-cli

Commands:
  db migrate
  db reset
  repo add <path> [name]
  repo list
  review create <repo_id> <base_ref> <head_ref> [title]
  review context <session_id>
  review record-fix <session_id> <commit_sha> [response_file|-] [tests_json_file]
  hook agent-stop
  export trajectory [repo_id]
  serve
"
    );
}
