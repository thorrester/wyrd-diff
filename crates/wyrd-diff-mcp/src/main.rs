...
impl McpServer {
    ...
    fn call_tool(&self, params: Value) -> std::result::Result<Value, String> {
        ...
        let value = match name {
            "wyrd_diff.register_repo" => self.register_repo(args),
            "wyrd_diff.create_review_session" => self.create_review_session(args),
            "wyrd_diff.list_review_sessions" => self.list_review_sessions(args),
            "wyrd_diff.open_review_session" => self.open_review_session(args),
            "wyrd_diff.get_agent_context" => self.get_agent_context(args),
            "wyrd_diff.get_open_comments" => self.get_open_comments(args),
            "wyrd_diff.pending_feedback" => self.pending_feedback(args),
            "wyrd_diff.import_fix" => self.import_fix(args),
            "wyrd_diff.export_trajectory" => self.export_trajectory(args),
    ...
    fn pending_feedback(&self, args: Value) -> Result<Value> {
        let session_id = self.resolve_session_id(&args)?;
        ...
        if let Some(ids) = args.get("resolves_thread_ids").and_then(Value::as_array) {
            for value in ids {
                if let Some(thread_id) = value.as_str()
                    && self.db.resolve_thread(thread_id)?
                {
    ...
    fn resolve_session_id(&self, args: &Value) -> Result<String> {
